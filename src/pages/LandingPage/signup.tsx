        ...
        <CardContent>
          <div className="space-y-4">
...

            {
              verificationPanel ? <>
                <div className="space-y-2">
                  <Label htmlFor="passcode" className="text-white">OTP</Label>
            ...
            <Button
              className="w-full"
              onClick={async () => { verificationPanel ? await otpHandler() : await signup() }}
            ...
            >
              {
                requesting ? <div className="flex items-center justify-center gap-3">
                  ...
                  <div>
                    {!verificationPanel ? "Create Account" : "Confirm OTP"}</div>
              }

            </Button>

            {
              verificationPanel && <Button variant={"outline"} className="w-full" disabled={
                requesting
              } onClick={async () => { await resendOtp() }}>{
                  requesting && verificationPanel ? <div className="flex items-center justify-center gap-3">
                    Please Wait
                    <Triangle
                      visible={true}
                      height="35"
                      ...
                      wrapperClass=""
                    />
                  </div> : "Resend OTP to Email"
                }</Button>
            }

            <div className="text-white font-sm">
              Already have account? <Button color="tertiary" variant={"link"} style={{ color: "white" }} >
                <div onClick={() => window.location.href = "/login"}>Login</div>
              </Button>
            </div>

            <div className="text-white">
              {verificationPanel ? "Need to Create Account?" : "Need to Verify Account?"} <Button onClick={() => setVerificationPanel(r => !r)} style={{ color: "white" }} variant={"link"}>Click Here</Button>
            </div>
