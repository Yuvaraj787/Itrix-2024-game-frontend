...
const Box = ({ data }: { data: any }) => {
  return (
    <div
      className={
        "flex temp flex-col items-center gap-4 h-max text-white w-max bg-red-100 p-8 rounded-lg shadow-md " +
        (data.rank === 1 ? "border border-yellow-300 border-2" : "mb-6")
      }
    >
      <div className="flex flex-col items-center">
        {data.rank === 1 && (
          <img style={{ height: "5rem" }} src="crown.svg"></img>
        )}
...
const App = () => {
  ...
  useEffect(() => {
    ...
    window.addEventListener("resize", () => {
      handleWindowSizeChange(temp1, temp2);
    });
    return () => {
  ...
  return (
    <div className="bg-gray-800 p-4 min-h-screen min-w-screen flex flex-col p-4 gap-4 pt-8 items-center">
      <Confetti
      ...
      <div>
        <button
          ...
          className="bg-red-600 shadow-xl hover:bg-indigo-500 text-white font-bold rounded-lg p-2 "
        >
          View Player-wise Team
        </button>
      </div>
      ...
      <div>
        <button
          onClick={() => navigate("/")}
          className="bg-violet-600 mt-2 shadow-xl hover:bg-indigo-500 text-white font-bold rounded-lg p-2 "
        >
          Play Again{" "}
        </button>
      </div>
      ...
      >
        <button className="bg-black p-2 text-white" onClick={closeModal}>
          Go Back
        </button>
          ...
          {data.length > 0 &&
            data.map((item: any) => {
              return (
                <button
                  className={
                    "p-2 flex items-center gap-4 capitalize border-black border-2 shadow-lg rounded-full " +
                    (Object.keys(viewData).length &&
                      (item.rank == viewData[0].rank
                        ? "bg-black text-white"
                        : ""))
                  }
                  onClick={() =>
            ...
            <div>
              ...
              <div className="bg-yellow-200 p-4 rounded-lg my-4">
                {viewData[0].players.map((item: any) => {
                  return <div className="font-bold capitalize">{item}</div>;
                })}
              </div>
